...
- (Task 109) 17 体目キャラ `Assets/Characters/fighter017.json`（"Shion"・HP1000・**スーパー必殺技を持つ正統派 shoto 型**の violet `[150,90,230]`）＋スプライト `fighter017.png`（256×896）を追加。`Character` の JSON 仕様は不変（16 体目 fighter016 に続く 17 体目＝ロスター 17 体）。アーキタイプ：飛び道具 `spirit_shot`（HADOUKEN）＋無敵リバーサル `rising_palm`（CHARGE_SHOT・`invincibleFrames`＋`launch`）＋**多段ビームスーパー `radiant_beam`（`superMove:true`・`hits:5`/`hitGap:3`＝5×44＝合計 220・幅 220 の打撃スーパー・Task 108）**、低段 `sweep`（knockdown）。撮影は `-x p1char=fighter017 -x p1meter=100` で 236236+攻撃。
