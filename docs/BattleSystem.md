...
- **ズームイン演出（Task 138）**：イントロ中はカメラをわずかに寄せ（`camera.zoom < 1`）、イントロ経過に従って通常倍率（1.0）へズームアウトして開始の勢いを出す。`GameRenderer.applyRoundIntroZoom(round)` を `camera.update()` 直前（hit shake のオフセット適用の後）に呼び、`progress = 1 - getIntroCountdown()/ROUND_INTRO_FRAMES` を 0..1 にクランプして `zoom = ROUND_INTRO_ZOOM + (1 - ROUND_INTRO_ZOOM) * progress`（寄り→等倍）を設定。イントロ中でなければ等倍。`centerCamera()`（タイトル/キャラ/ステージ選択）でも `camera.zoom=1` に戻して非バトル画面へ持ち越さない。撮影は既定でイントロがスキップされるため**既存レシピ不変**（`-x intro=true` 指定時のみズームが映る）。純描画でシミュレーション非干渉＝決定的・リプレイ不変。
...
- (Task 138) ラウンド開始のズームイン演出（round intro zoom）を追記。`GameRenderer.ROUND_INTRO_ZOOM`(0.82)＋`applyRoundIntroZoom(round)` を `camera.update()` 直前で呼び、`round.isRoundIntro()` 中は `progress=1-getIntroCountdown()/ROUND_INTRO_FRAMES`（0..1 クランプ）から `zoom=ROUND_INTRO_ZOOM+(1-ROUND_INTRO_ZOOM)*progress`（寄り→等倍）を設定、イントロ外は等倍。`centerCamera()` でも `camera.zoom=1` に戻し非バトル画面へ持ち越さない。撮影は既定でイントロスキップ＝既存レシピ不変（`-x intro=true` でのみ映る）。純描画でシミュレーション非干渉＝決定的・リプレイ不変。「ラウンド開始イントロ（Task 42）」節に「ズームイン演出（Task 138）」を追記・冒頭サマリを更新。
