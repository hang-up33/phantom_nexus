...
PC に接続されたコントローラーで操作できる（公式拡張 **gdx-controllers**・`Infra/Build/build.gradle` に core + desktop を追加）。設計の要点：

- **キーボード抽象の単一の真実は不変**：入力は引き続き `InputAction`（論理アクション）で扱い、ゲームロジック（移動 / コマンド / AI 差し替え）は物理デバイスを知らない。コントローラーは `PlayerInput.isDown`/`isPressed` に **OR 合成**するだけ（キーボードと併用可・コマンド入力＝波動拳/ダッシュもスティック/十字キーでそのまま成立）。
- **集約クラス `GamepadInput`（`GameRuntime/Input`）**：gdx-controllers をラップし、毎フレーム `poll()`（`render()` 先頭で 1 回）で全コントローラーの押下状態と**立ち上がりエッジを前フレーム差分で自前計算**する（ポーリング API には just-pressed が無いため）。標準ゲームパッドのマッピング（`ControllerMapping`）で射影：左スティック/D-pad＝移動・しゃがみ・ジャンプ、A=弱・B=中・X(またはR1)=強・Y(またはL1)=投げ、START/BACK＝メニュー決定/戻る。デッドゾーン 0.5。
- **スロット**：接続順に slot0=1P / slot1=2P。`PlayerInput.attachGamepad(gamepad, slot)` で接続。
- **後方互換・決定性**：拡張未導入/ネイティブ読込失敗/ヘッドレスでは `poll()` が例外を握りつぶして以後 no-op＝キーボードのみで従来どおり動く。**撮影モード・リプレイ再生では `gamepadActive=false` で `poll()` 自体を呼ばない**（`poll()` が唯一の状態更新点なので、ポーリングしなければ押下集合/エッジは空のまま＝**メニュー含む全クエリが false に縮退**し、コントローラーが繋がっていても決定的再現を一切乱さない）。**記録モードでは有効**＝コントローラー操作も記録される。乱数なし。
- **メニュー**：タイトル/キャラ選択/ステージ選択/決着後のタイトル復帰も `gamepad.menuUp/Down/Left/Right/Confirm/Cancel()`（全スロット OR）で操作できる。
...
- **ダッシュ開始の砂煙（Task 140）**：地上ダッシュ（前ステップ / バックステップ）の開始でも足元に砂煙を出す。前フレームのダッシュ状態 `pXWasDashing` を持ち、被弾処理より前に `detectDashStart(f, wasDashing)`＝`!wasDashing && f.isDashing() && f.isGrounded()`（空中ダッシュは滞空なので地上の砂煙を出さない）で判定し `GameConstants.GROUND_Y` に `LandingDust` を生成。`resetFighters` で `pXWasDashing=false`。着地 / 踏み切りと同じ `LandingDust` を流用＝新 POJO / 新描画なし・決定的。
...
## 必殺技ゲージ満タンのオーラ（meter-full aura）（Task 137）

必殺技ゲージが満タン（EX 必殺技・スーパー必殺技が撃てる状態）のファイターの足元に、金色のパルスする光輪を出す「発動可能」インジケータの演出。HUD のメーターバー（金色点灯・{@code drawSuperMeter}）と連動し、キャラ本体側でも一目で分かるようにする。**純粋な視覚演出で戦闘結果には影響しない**（描画のみ）。

- **描画（`GameRenderer`）**：足元の影（Task 130）と同じくスプライト描画（パス 2）の前（パス 1）に、`f.hasFullMeter()` のとき床へ金色の楕円（`METER_AURA_COLOR`）を描く。`auraTick`（`renderScene` で毎フレーム +1 する描画フレームカウンタ）から `sin` で横径・不透明度を周期（`METER_AURA_PERIOD`）で脈動させる。満タンでなければ何も描かない＝従来どおり（後方互換）。
- **決定性**：メーター量（決定的）と描画フレームカウンタから定まる**乱数なし**のパルス。シミュレーションに非フィードバック＝**リプレイ / スクショレシピ不変**（メーター遷移自体は既存ロジックのまま）。

---

## 決着演出：KO 白フラッシュ / 勝者グロー / 勝利の光の粒（Task 148/149/150）

ラウンド決着・ラウンド間の盛り上げ演出群。**いずれも純描画で戦闘結果に影響しない**（表示のみ・乱数なし＝決定的）。`GameRenderer.drawRoundEndOverlays`（パス 3 末尾）に集約。

- **KO 白フラッシュ（Task 148）**：KO で決着した瞬間（`renderScene` で `round.isFinished()||isBetweenRounds()` が `false→true` かつ `getReason()==KO` の立ち上がり）に `koFlashFrames` をアームし、その間**画面の縁を白く**フラッシュして残りフレーム比でフェードする。タイムアップ決着では光らせない。
- **勝者グロー（Task 149）**：決着 / ラウンド間に `getRoundWinner()` が P1/P2 のとき、勝者の足元へ金色のパルス光輪（`drawWinnerGlow`・メーター満タンオーラと同型で大きめ・金色）を出す。引き分けは無し。
- **勝利の光の粒（Task 150）**：勝者の周囲に金色の光の粒が舞い上がる祝祭演出（`drawVictorySparkles`）。粒の位置は粒番号と描画フレームカウンタ `auraTick` からの固定計算（背景の浮遊パーティクル Task 139 と同型・上向き・金色・乱数なし）。
- **実装上の注意（ソフトウェア GL の癖）**：本環境のヘッドレス・ソフトウェア GL（Mesa/llvmpipe）では**全画面ソリッドの半透明 rect、特に半透明の「黒」が想定どおり暗転にならない**（明るく化ける）。そのため暗転系は使わず、KO フラッシュは**縁グラデーション rect**（`drawEdgeVignette`・edge→`Color.CLEAR`・低 HP ビネット Task 145 と同形式で動作実績あり）、グロー / 粒は**楕円 / 円**（非黒・動作実績あり）で構成している。
- **決定性**：決着状態（決定的）と `auraTick` のみ＝乱数なし・シミュレーション非フィードバック＝**リプレイ / スクショレシピ不変**。

---

## 低 HP 警告ビネット（low-HP vignette）（Task 145）

どちらかのファイターの残量割合が低い（既定 25% 以下）とき、画面の四辺を赤くフェードさせて脈動させ危機感を出す演出。**純描画で戦闘結果に影響しない**（表示のみ）。

- **描画（`GameRenderer.drawLowHpVignette`）**：パス 3 末尾（最前面寄り）で、`min(p1, p2 の HP 割合) <= LOW_HP_RATIO` のとき四辺に「辺＝赤 / 内側＝透明」のグラデーション rect を描く。不透明度は残量が低いほど・`auraTick` の `sin` 脈動の山ほど強い。十分な残量なら何も描かない。
- **撮影**：`-x p1hp=<値>` / `-x p2hp=<値>`（新オーバーライド）で初期 HP を下げて削り合わず撮れる。
- **決定性**：HP 割合＋描画カウンタのみ＝乱数なし・シミュレーション非フィードバック＝**リプレイ / スクショレシピ不変**。

---

## HP バーのダメージトレイル（Task 146）

被弾で減った HP 分を、HP バー上に明るい色の**遅延バー**で一瞬残し、どれだけ減ったかを伝える演出（格闘 / 対戦ゲーム定番）。**純描画**（HUD 表示のみ）。

- **描画（`GameRenderer.drawHpBar`）**：ファイターごとの遅延割合 `hpTrail[]`（p1/p2）を実 HP 割合へ毎フレーム `HP_TRAIL_DRAIN` ずつ詰める。割合が増加（回復 / ラウンド開始）なら即追従、減少（被弾）でのみ遅れる。`hpTrail > 実割合` の差分を失った側に明るい黄（`HP_TRAIL_COLOR`）で描く（白フィルの隣・回復可能ライフの下層）。
- **決定性**：HP 割合と描画フレーム進行のみ＝乱数なし・シミュレーション非フィードバック＝表示専用（リプレイ / スクショレシピ不変）。

---

## 空の光の帯（sky sweep）（Task 147）

背景（空）をゆっくり横切る淡い光の縦帯で空気感を出す演出。浮遊パーティクル（Task 139）と同じ背景レイヤ。**純描画**。

- **描画（`GameRenderer.drawSkySweep`）**：パス 1（スプライトの前）で、中心 X が `auraTick × SKY_SWEEP_SPEED` で横移動し画面幅 + 帯幅で wrap する縦帯を、中央が最も明るい 3 枚の rect 近似で描く。乱数なし・決定的。

---

## 背景の浮遊パーティクル（ambient motes）（Task 139）

背景（空＝地面より上の領域）に微かな光の粒をゆっくり上昇・横揺れさせ、空気感・奥行きを出す演出。全ステージ共通。**純粋な視覚演出で戦闘結果には影響しない**（背景描画のみ）。

- **描画（`GameRenderer.drawAmbientMotes`）**：スプライト描画（パス 2）の前（パス 1）に呼び、`MOTE_COUNT` 個の粒を描く。各粒の横位置は「粒番号で散らした基準 X＋`sin` の横揺れ」、縦位置は「粒ごとの初期高さ＋`auraTick × MOTE_RISE` を空の高さで wrap（下→上へ流れ天井で巻き戻る）」、半径は粒番号でわずかに変える。すべて粒番号と描画フレームカウンタ（`auraTick`）からの固定計算＝**乱数なし**。
- **決定性**：描画カウンタのみに依存し戦闘状態へ非フィードバック＝**リプレイ / スクショレシピ不変**・決定的。

---

## 残り時間警告の点滅（low time warning）（Task 141）

ラウンドの残り時間が少なくなると、中央上のタイマー数字を赤く拡大して脈動させ緊張感を出す演出。**純粋な視覚演出**（表示のみ・タイマー値や勝敗ロジックは不変）。

- **描画（`GameRenderer`）**：`round.getRemainingSeconds() <= LOW_TIME_THRESHOLD`（既定 10 秒）かつ戦闘中（決着・ラウンド間でない）のとき、タイマーフォントを赤（`LOW_TIME_COLOR`）と白の間で `auraTick` の `sin` で脈動させ、倍率も少し大きくする。描画後に色・倍率を既定（白・等倍）へ戻す。
- **決定性**：残り秒数（決定的）と描画カウンタのみ＝**乱数なし**・シミュレーション非フィードバック＝**リプレイ / スクショレシピ不変**。

---

## クリーンヒットの白フラッシュ（impact flash）（Task 136）

打撃 / 投げ / ダウンの**ダメージ被弾直後**、被弾側を数フレーム発光させて命中の手応えを強める game feel の演出。ヒットスパーク（接触点）・ヒットストップ（凍結）・画面振動（カメラ）と重なり衝撃が増す。**純粋な視覚演出で戦闘結果には影響しない**（表示専用カウンタのみ・HP / 位置 / 当たり判定とは独立）。

- **状態（`Fighter`）**：表示専用カウンタ `impactFlashFrames` を持ち、ダメージを与える 3 経路（`applyHit` / `applyThrow` / `applyKnockdown`）で `GameConstants.IMPACT_FLASH_FRAMES`（既定 4）にセット、`update()` 冒頭で毎フレーム減衰、`reset()` で 0。`getImpactFlashFrames()` で公開。ガード chip（`applyGuard`）・投げ抜け（`applyThrowTech`）はノーダメージなので発光しない。
- **描画（`GameRenderer`）**：スプライトキャラは `drawFighterSprite` で本体描画の後に**加算合成**（`GL_SRC_ALPHA, GL_ONE`）でスプライトをもう一度描いてシルエットを発光させる（透明余白を光らせない＝矩形に見えない・強さ `IMPACT_FLASH_PEAK` を残りフレーム比で減衰・描画後に既定の α ブレンドへ戻す）。矩形フォールバックキャラは本体矩形 = キャラなので `drawFighterOverlay` の fallback 分岐で白を上重ねする。
- **決定性**：被弾（決定的）で発火し残りフレームから強さが決まる**乱数なし**＝入力リプレイと両立。表示専用でシミュレーションに非フィードバック＝**リプレイ / スクショレシピ不変**。
...
- **ズームイン演出（Task 138）**：イントロ中はカメラをわずかに寄せ（`camera.zoom < 1`）、イントロ経過に従って通常倍率（1.0）へズームアウトして開始の勢いを出す。`GameRenderer.applyRoundIntroZoom(round)` を `camera.update()` 直前（hit shake のオフセット適用の後）に呼び、`progress = 1 - getIntroCountdown()/getIntroTotalFrames()` を 0..1 にクランプして `zoom = ROUND_INTRO_ZOOM + (1 - ROUND_INTRO_ZOOM) * progress`（寄り→等倍）を設定。イントロ中でなければ等倍。`centerCamera()`（タイトル/キャラ/ステージ選択）でも `camera.zoom=1` に戻して非バトル画面へ持ち越さない。撮影は既定でイントロがスキップされるため**既存レシピ不変**（`-x intro=true` 指定時のみズームが映る）。純描画でシミュレーション非干渉＝決定的・リプレイ不変。
...
## 画面フロー：決着後にスタート画面（タイトル）へ戻る

マッチ確定（`RoundManager.isFinished()`）後は `update()` が全更新を凍結して結果バナーを静止表示するが、そこから **`ENTER`/`SPACE`/`J`/`ESC` でタイトル画面へ戻れる**（連戦やモード変更のため）。

- `render()` の BATTLE 処理冒頭で `round.isFinished()` かつ通常プレイ（撮影/リプレイでない）なら上記キーで `returnToTitle()`（`screen = TITLE`・`titleSelection = 0`）。遷移したフレームは `return` して次フレームからタイトルを処理する（メニュー遷移と同じ作法＝確定入力の再消費を防ぐ）。
- 再開時のバトル構築は各遷移が改めて行うため前マッチの確定状態を持ち越さない：対戦＝キャラ/ステージ選択→`startBattle`、トレーニング＝`startTraining`（`new RoundManager` ＋ `resetFighters()` でラウンド・ファイターを作り直してから `BATTLE`）。
- **後方互換**：撮影モード・リプレイ（記録/再生）ではこの復帰を無効化し、結果バナーを従来どおり凍結表示する（既存スクショレシピ・リプレイの決定性を壊さない）。
- 結果バナーに `PRESS ENTER TO RETURN TO TITLE` のヒントを表示する（`GameRenderer.setReturnToTitleHint`）。通常プレイのマッチ確定時のみ表示し、撮影は既定で非表示（既存の結果バナー・PERFECT 等のスクショレシピ不変）。証跡用に `-x returntotitle=true` で撮影時もヒントを重ねて撮れる。
...
- (ゲームパッド対応) コントローラー入力を追記。公式拡張 **gdx-controllers**（core + desktop・`Infra/Build/build.gradle`）を追加し、`GameRuntime/Input/GamepadInput` で集約。`render()` 先頭で `poll()`（前フレーム差分でエッジを自前計算）し、`PlayerInput.attachGamepad(slot)` でキーボードと OR 合成（`isDown`/`isPressed`）。標準マッピング（左スティック/D-pad＝移動・しゃがみ・ジャンプ、A/B/X(R1)/Y(L1)＝弱/中/強/投げ、START/BACK＝メニュー決定/戻る・デッドゾーン 0.5）。タイトル/キャラ選択/ステージ選択/決着後タイトル復帰も `menu*()` で操作可。接続順に 1P/2P。**後方互換・決定性**：拡張未導入/ネイティブ失敗/ヘッドレスは `poll()` が例外を握りつぶし no-op＝キーボードのみ（従来挙動）。撮影モード・リプレイ再生では `gamepadActive=false` で `poll()` を呼ばず全クエリ（メニュー含む）が false に縮退＝決定的再現を維持／記録モードでは有効＝操作も記録。軸の未マッピング（-1）も `readAxis` でガード。乱数なし。`Fighter`/`CollisionSystem`/`GameConstants`/JSON スキーマは不変（入力層の追加のみ）。「ゲームパッド入力（コントローラー対応）」節を追加。
- (Task 148/149/150) 決着演出（KO 白フラッシュ / 勝者グロー / 勝利の光の粒）を追記。`GameRenderer.drawRoundEndOverlays`（パス 3 末尾）に集約：`koFlashFrames`/`prevConcluded`（`renderScene` で KO 決着の立ち上がりを検出してアーム）＋`KO_FLASH_*`、`drawWinnerGlow`＋`WINNER_GLOW_*`、`drawVictorySparkles`＋`VICTORY_SPARKLE_*`、汎用 `drawEdgeVignette`（縁グラデ）。低 HP ビネット（Task 145）は決着中は出さないよう `!concluded` ゲートを追加。**本環境のソフトウェア GL では全画面ソリッド半透明 rect（特に黒）が暗転にならず明るく化けるため**、暗転系を避け縁グラデ（KO 白フラッシュ）＋楕円/円（金グロー・金の粒）の非黒構成にした。決着状態＋`auraTick` のみ＝乱数なし・表示専用＝**リプレイ/スクショレシピ不変**・決定的。「決着演出：KO 白フラッシュ / 勝者グロー / 勝利の光の粒（Task 148/149/150）」節・冒頭サマリを追加。
- (Task 147) 空の光の帯（sky sweep）を追記。`GameRenderer.drawSkySweep`＋定数 `SKY_SWEEP_COLOR`/`SKY_SWEEP_WIDTH`(260)/`SKY_SWEEP_ALPHA`(0.16)/`SKY_SWEEP_SPEED`(2.4)＋作業色 `sweepColor`。パス 1（スプライトの前）で、中心 X が `auraTick×SPEED` で横移動し画面幅+帯幅で wrap する縦帯を 3 枚の rect 近似（中央最明）で描く。乱数なし・決定的・リプレイ/スクショ不変。「空の光の帯（sky sweep）（Task 147）」節・冒頭サマリを追加。
- (Task 146) HP バーのダメージトレイルを追記。`GameRenderer` に `hpTrail[2]`＋定数 `HP_TRAIL_COLOR`/`HP_TRAIL_DRAIN`(0.010)。`drawHpBar` で遅延割合を実 HP 割合へ毎フレーム詰め（増加は即追従・減少のみ遅延）、差分を失った側に明るい黄で描く。HP 割合＋描画進行のみ＝乱数なし・表示専用・決定的。「HP バーのダメージトレイル（Task 146）」節・冒頭サマリを追加。
- (Task 145) 低 HP 警告ビネットを追記。`GameRenderer.drawLowHpVignette`＋定数 `LOW_HP_RATIO`(0.25)/`LOW_HP_VIGNETTE_COLOR`/`LOW_HP_VIGNETTE_BAND`(130)/`LOW_HP_VIGNETTE_ALPHA`(0.5)＋作業色 `vignetteColor`。パス 3 末尾で `min(p1,p2 の HP 割合) <= 0.25` のとき四辺を「辺=赤/内側=透明」グラデで描き、残量が低いほど・`auraTick` の sin 脈動の山ほど濃く。撮影用に初期 HP オーバーライド `p1hp`/`p2hp`（`ScreenshotController.initialHp`＋`Fighter.setCurrentHp`＋build.gradle 転送リスト）を追加。HP 割合＋描画カウンタのみ＝乱数なし・表示専用・決定的。「低 HP 警告ビネット（low-HP vignette）（Task 145）」節・冒頭サマリを追加。
- (Task 144) 飛び道具発射のマズルフラッシュを追記。`PhantomNexusGame.spawnProjectile` の弾生成直後に `spawnHitSpark(false, spawnX, spawnY+height/2)` を 1 つ出す（HitSpark を流用・新規型なし）。純演出・乱数なし・決定的。
- (Task 143) コンボカウンターの拡大パルスを追記。`GameRenderer.drawComboCounter` の "N HITS!" 倍率を `COMBO_SCALE * (1 + COMBO_PULSE * sin(auraTick * COMBO_PULSE_SPEED))` で小刻みに脈動。描画フレームカウンタのみ＝乱数なし・表示専用・決定的。
- (Task 142) ダメージポップアップのダメージ比例拡大を追記。`GameRenderer.drawDamagePopups` でポップアップごとに `POPUP_SCALE * (POPUP_MIN_SCALE_FACTOR + (1-…) * min(amount, POPUP_SCALE_DAMAGE_REF)/POPUP_SCALE_DAMAGE_REF)` の倍率を設定（大ダメージほど大きく）。与ダメージ量のみ＝乱数なし・表示専用・決定的。`setScale` をループ内に移し描画後に等倍へ戻す。
- (Task 141) 残り時間警告の点滅（low time warning）を追記。`GameRenderer.LOW_TIME_THRESHOLD`(10)/`LOW_TIME_COLOR`(赤)/`LOW_TIME_PULSE_SPEED`＋作業色 `timerColor`。`round.getRemainingSeconds() <= 10` かつ戦闘中のとき、タイマー数字を赤⇔白で `auraTick` の `sin` 脈動＋拡大し、描画後に既定（白・等倍）へ戻す。残り秒数＋描画カウンタのみ＝乱数なし・表示専用でシミュレーション非フィードバック＝**リプレイ/スクショレシピ不変**・決定的。撮影は `-x timelimit=8` 等。「残り時間警告の点滅（low time warning）（Task 141）」節・冒頭サマリを追加。
- (Task 140) ダッシュ開始の砂煙（dash dust）を追記。`PhantomNexusGame` に前フレームのダッシュ状態 `p1WasDashing`/`p2WasDashing`＋`detectDashStart(f, wasDashing)`＝`!wasDashing && f.isDashing() && f.isGrounded()`（地上ダッシュ開始のみ・空中ダッシュ除外）を `detectLanding`/`detectTakeoff` と並べて判定し `GameConstants.GROUND_Y` に `LandingDust` を生成、`resetFighters` で `false` に戻す。着地（131）/踏み切り（135）と同じ `LandingDust` 流用＝新 POJO/新描画なし・乱数なし・決定的。「着地の砂煙（Task 131）」節に「ダッシュ開始の砂煙（Task 140）」を追記・冒頭サマリを更新。
- (Task 139) 背景の浮遊パーティクル（ambient motes）を追記。`GameRenderer.drawAmbientMotes`＋定数 `MOTE_COLOR`/`MOTE_COUNT`(30)/`MOTE_RADIUS`/`MOTE_DRIFT`/`MOTE_RISE`/`MOTE_DRIFT_SPEED`。パス 1（スプライトの前）で、各粒の横位置＝散らした基準 X＋`sin` 横揺れ・縦位置＝初期高さ＋`auraTick×MOTE_RISE` を空の高さで wrap・半径＝粒番号で微変、と粒番号と `auraTick` からの固定計算で描く。乱数なし・戦闘状態へ非フィードバック＝**リプレイ/スクショレシピ不変**・決定的。「背景の浮遊パーティクル（ambient motes）（Task 139）」節・冒頭サマリを追加。
- (Task 138) ラウンド開始のズームイン演出（round intro zoom）を追記。`GameRenderer.ROUND_INTRO_ZOOM`(0.82)＋`applyRoundIntroZoom(round)` を `camera.update()` 直前で呼び、`round.isRoundIntro()` 中は `progress=1-getIntroCountdown()/getIntroTotalFrames()`（0..1 クランプ）から `zoom=ROUND_INTRO_ZOOM+(1-ROUND_INTRO_ZOOM)*progress`（寄り→等倍）を設定、イントロ外は等倍。`centerCamera()` でも `camera.zoom=1` に戻し非バトル画面へ持ち越さない。撮影は既定でイントロスキップ＝既存レシピ不変（`-x intro=true` でのみ映る）。純描画でシミュレーション非干渉＝決定的・リプレイ不変。「ラウンド開始イントロ（Task 42）」節に「ズームイン演出（Task 138）」を追記・冒頭サマリを更新。
- (Task 137) 必殺技ゲージ満タンのオーラ（meter-full aura）を追記。`GameRenderer` に描画フレームカウンタ `auraTick`（`renderScene` で +1）＋作業色 `auraColor`、定数 `METER_AURA_COLOR`（金）/`METER_AURA_WIDTH_SCALE`(1.45)/`METER_AURA_HEIGHT`(20)/`METER_AURA_PULSE`(0.22)/`METER_AURA_PERIOD`(36)。`drawMeterAura(f)` を影（パス 1）と並べて呼び、`f.hasFullMeter()` のとき足元に金色の楕円を `sin` パルス（横径・α を周期脈動）で描く。満タンでなければ非描画＝後方互換。メーター量＋描画カウンタから定まる乱数なしのパルス・シミュレーション非フィードバック＝**リプレイ/スクショレシピ不変**・決定的。「必殺技ゲージ満タンのオーラ（meter-full aura）（Task 137）」節・冒頭サマリを追加。
- (Task 136) クリーンヒットの白フラッシュ（impact flash）を追記。`GameConstants.IMPACT_FLASH_FRAMES`(4)＋`Fighter.impactFlashFrames`（表示専用カウンタ）。ダメージ 3 経路（`applyHit`/`applyThrow`/`applyKnockdown`）でセット、`update()` 冒頭で減衰、`reset()` で 0、`getImpactFlashFrames()` 公開。ガード chip / 投げ抜けはノーダメージなので非発光。`GameRenderer` はスプライトキャラを `drawFighterSprite` で加算合成（`GL_SRC_ALPHA, GL_ONE`・強さ `IMPACT_FLASH_PEAK`=0.85 を残りフレーム比で減衰・描画後に既定 α ブレンドへ戻す）でシルエット発光させ、矩形フォールバックは fallback 分岐で白（`IMPACT_FLASH_COLOR`）を上重ね。被弾（決定的）で発火・乱数なし・表示専用でシミュレーション非フィードバック＝**リプレイ/スクショレシピ不変**・決定的。「クリーンヒットの白フラッシュ（impact flash）（Task 136）」節・冒頭サマリを追加。
