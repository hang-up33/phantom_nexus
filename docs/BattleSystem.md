...
## 簡易 AI（Task 21 → Task 37 → Task 50 → Task 51）
...
- **決定的**：判断は相手の状態（`isThrowing()`）＋距離＋自分の接地/行動可否のみで**乱数なし**（入力リプレイと両立）。AI 自身のジャンプ・必殺技・しゃがみ系は将来拡張。
