...
- **後方互換・決定性**：拡張未導入/ネイティブ読込失敗/ヘッドレスでは `poll()` が例外を握りつぶして以後 no-op＝キーボードのみで従来どおり動く。**撮影モード・リプレイ再生では接続しない**（強制入力/記録済み入力のみで決定的に再現するため）。**記録モードでは接続する**＝コントローラー操作も記録される。乱数なし。
