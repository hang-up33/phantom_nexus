...
- **AI（Task 21 / 37 / 50 / 51）**：接近して間合いで攻撃する簡易 AI（P2 既定）に、**読み合い反応**を追加。相手の打撃に合わせて**ガード**（chip で凌ぐ）し、相手が**ガード中**なら打撃では崩せないので**投げ**で崩す。**Task 51** で**投げ抜け反応**を追加：相手が**投げ中**で近接なら投げ抜け窓をアームして掴みを**抜ける**（ノーダメージ）＝「打撃＝ガード／ガード＝投げ／投げ＝投げ抜け」の三すくみが完成（ただし AI の技の硬直を投げで狩る崩しは残る）。さらに **Task 50** で**ダッシュ接近**：中心間が遠い（> 260px）ときは歩きでなく**ダッシュ**（二度押し前ステップ）で素早く間合いを詰める。判断は相手の観測状態と距離のみで**乱数なし＝決定的**（入力リプレイと両立）。AI 自身のジャンプ・必殺技・しゃがみ系は将来拡張
