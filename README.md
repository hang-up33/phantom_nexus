...
- **必殺技ゲージ／EX 必殺技（Task 44）**：攻撃を当てる/受ける/ガードで必殺技ゲージ（画面下端に表示）が貯まり、満タン（金色）で必殺技（飛び道具）を撃つと消費して EX 版（ダメージ 1.6 倍・大型の金色弾）になる。ガードゲージ（防御）と対になる攻撃リソース。乱数なしで決定的（撮影は `-x p1meter=` でオーバーライド可）
