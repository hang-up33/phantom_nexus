...
`./gradlew run` で **タイトル → モード選択 →（対戦なら）キャラクター選択 → ステージ選択 → バトル** と進む。決着後は結果画面で **`ENTER`/`SPACE`/`J`/`ESC` を押すとタイトルへ戻れる**（連戦・モード変更用）。

| タイトル（デモリール背景） | キャラクター選択（立ち絵プレビュー） |
|---|---|
| ![タイトル](docs/screenshots/160-title.png) | ![キャラクター選択](docs/screenshots/161-charselect.png) |
| 全 10 ステージの実背景をデモリール巡回＋ **VERSUS / TRAINING** 選択 | 全 10 キャラからスプライト立ち絵を見ながら P1 / P2 を選択 |

| ステージ選択（実背景ライブプレビュー） | 対戦画面（モダンな見た目） |
|---|---|
| ![ステージ選択](docs/screenshots/159-stageselect.png) | ![対戦画面](docs/screenshots/170-showcase.png) |
| 全 10 ステージの多層背景をライブプレビュー＝カーソルで切替 | 多層パララックス背景＋情景アニメ＋エネルギー飛び道具・クリーン HUD（当たり判定の箱は F1 デバッグ時のみ） |
...
PC に接続されたコントローラーでもプレイできる（公式拡張 `gdx-controllers`）。接続は**自動検出**で、繋いでいなければ従来どおりキーボードで操作できる（両者は併用可）。接続順に **1 本目＝1P / 2 本目＝2P** を操作する。

| 操作 | ボタン |
|---|---|
| 移動 / しゃがみ / ジャンプ | 左スティック または 十字キー（上＝ジャンプ・下＝しゃがみ） |
| 弱攻撃（light） | A |
| 中攻撃（medium） | B |
| 強攻撃（heavy） | X（または R1） |
| 投げ（throw） | Y（または L1） |
...
※ ボタン名は標準ゲームパッドのマッピング（Xbox 配列）準拠。波動拳・ダッシュ等のコマンド入力もスティック/十字キーでそのまま出せる。タイトル/キャラ選択/ステージ選択もコントローラーで操作可能。
...
| 175 | BGM（バックグラウンドミュージック）：タイトル/バトル/リザルト用 OGG をループ再生・F6 でミュートトグル（M キー=SE と独立）・画面遷移で自動切替（`GameRuntime/Audio/MusicManager`）。OGG 未配置はサイレントフォールバック（後方互換） | ✅ 完了 |
| 176 | アーケードモード：タイトル画面に ARCADE を追加・ランダムキャラ/ステージで 10 連戦のストーリー対戦・クリア＋敗退メッセージ・通常対戦/トレーニングと完全独立 | ✅ 完了 |
| 177 | セッション戦績：アプリ起動中のマッチ結果を P1W-L / P2W-L 形式で累積し決着バナーに表示（非撮影/非リプレイ時のみ・決定性不変） | ✅ 完了 |
| 178 | リマッチ：決着後に R キーで同キャラ/ステージ/モードで即再戦・黄色の「PRESS R TO REMATCH」ヒントを結果バナーに表示（通常プレイのみ・後方互換） | ✅ 完了 |
| 179 | ランダムキャラ選択：キャラ選択画面で R キーを押すとカーソルをランダム位置に移動→ ENTER で確定・「R : random」操作ヒントをキー説明に追加 | ✅ 完了 |
| 180 | ドキュメント総整備：CLAUDE.md フェーズ更新（Task 173〜179 反映）・撮影オーバーライド `rematch` 追記・README 進捗反映・kaizen 反映 | ✅ 完了 |
| 181 | バトル一時停止：P キーで試合を一時停止／再開（タイムカウントも停止・リプレイ記録/再生中は無効）。ESC でもポーズメニュー（RESUME / RETURN TO TITLE）を表示・ESC 再押しで再開・ゲームパッド対応（後方互換） | ✅ 完了 |
| 182 | キャラクタービューア：タイトル VIEWER 選択で全キャラのスプライトアニメを閲覧（LEFT/RIGHT でキャラ切替・UP/DOWN でアニメ状態切替・自動フレーム送り・ESC でタイトルへ戻る・撮影は `-x startscreen=viewer`） | ✅ 完了 |
| 183 | リプレイ保存・読込：バトル中の入力を毎フレーム in-memory バッファに記録し、マッチ終了時に `replays/last.rep` へ自動保存。タイトルに「REPLAY LAST」を追加し直前バトルを再生 | ✅ 完了 |
| 184 | サバイバルモード：タイトルに SURVIVAL を追加・P1 の HP を引き継ぎながらランダム相手と連戦・P2 撃破ごとに kills カウントアップ・タイトルメニューはリプレイ保存有無で 3〜4 項目に動的変化 | ✅ 完了 |
| 185 | タイムアタックモード：タイトルに TIME ATTACK を追加・60 秒の制限時間内にランダム相手を何人倒せるか・P1 勝利で即次の対戦相手（確認不要・HP リセット）・タイムアップ/敗北で GAME OVER | ✅ 完了 |
| 186 | アーケードスコア：サバイバル/タイムアタックのゲームオーバー時に結果バナーへ「SCORE: N」（kills × 1000）を金色で表示・`-x arcadescore=N` で撮影用オーバーライドも追加 | ✅ 完了 |
...
- **操作/移動**：キーボード ＋ ゲームパッド（コントローラー）入力抽象・左右移動・ジャンプ（重力/接地）・向き自動追従・しゃがみ（DOWN 押しで低姿勢）・しゃがみ移動（DOWN+左右で通常速度の半分でクロール）。コントローラーは自動検出（接続順に 1P/2P）で、キーボードと併用可・未接続でも従来どおり動く
