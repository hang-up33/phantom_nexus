...
- **AI（Task 21 / 37 / 50）**：接近して間合いで攻撃する簡易 AI（P2 既定）に、**読み合い反応**を追加。相手の打撃に合わせて**ガード**（chip で凌ぐ）し、相手が**ガード中**なら打撃では崩せないので**投げ**で崩す。さらに **Task 50** で**ダッシュ接近**を追加：中心間が遠い（> 260px）ときは歩きでなく**ダッシュ**（二度押し前ステップ）で素早く間合いを詰める。判断は相手の観測状態と距離のみで**乱数なし＝決定的**（入力リプレイと両立）。投げ抜け反応・ジャンプ・必殺技は将来拡張
