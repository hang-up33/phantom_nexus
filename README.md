...
`./gradlew run` で **タイトル → モード選択 →（対戦なら）キャラクター選択 → ステージ選択 → バトル** と進む。

| タイトル（デモリール背景） | キャラクター選択（立ち絵プレビュー） |
|---|---|
| ![タイトル](docs/screenshots/160-title.png) | ![キャラクター選択](docs/screenshots/161-charselect.png) |
| 全 10 ステージの実背景をデモリール巡回＋ **VERSUS / TRAINING** 選択 | 全 20 キャラからスプライト立ち絵を見ながら P1 / P2 を選択 |
...
| 117 | キャラクター選択画面（対戦選択時に遷移・全 18 キャラから P1/P2 を選択してバトル開始） | ✅ 完了 |
...
| 161 | キャラクター選択画面の刷新（選択中キャラのスプライト立ち絵を中央に大きくプレビュー＋確定 P1/P2 を左右に表示＋実ステージ背景＋下部に全20体グリッド・純 UI） | ✅ 完了 |
