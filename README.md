...
- **データ駆動**：キャラ/ステージを外部 JSON から読込（`Shared/Schema` の単一の真実 + バリデーション）。ステージは複数（`stage001` Twilight Arena / `stage002` Verdant Glade）を JSON だけで追加・差し替え可能（Task 40。撮影は `-x stage=<id>` でオーバーライド）
