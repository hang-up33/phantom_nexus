...
- **AI（Task 21 / 37 / 50 / 51）**：接近して間合いで攻撃する簡易 AI（P2 既定）に、**読み合い反応**を追加。相手の打撃に合わせて**ガード**（chip で凌ぐ）し、相手が**ガード中**なら打撃では崩せないので**投げ**で崩す。**Task 51** で**投げ抜け反応**を追加：相手が**投げ中**で近接なら投げ抜け窓をアームして掴みを**抜ける**（ノーダメージ）＝「打撃＝ガード／ガード＝投げ／投げ＝投げ抜け」の三すくみが完成（ただし AI の技の硬直を投げで狩る崩しは残る）。さらに **Task 50** で**ダッシュ接近**：中心間が遠い（> 260px）ときは歩きでなく**ダッシュ**（二度押し前ステップ）で素早く間合いを詰める。**Task 55** で**無敵対空**を追加：無敵リバーサル（打撃必殺技）を持つキャラの AI は、飛び込んでくる相手（空中＋下降中）をその無敵技で迎撃する（AI が初めて必殺技を使う・`self.startSpecial` を直接呼ぶ）。データ駆動で、キャラ JSON に無敵打撃技があれば AI もそれで対空する（fighter002 Akane の `rising_talon`）。**Task 56** で**難易度**（EASY/NORMAL/HARD）を追加：実装済みの反応群を段階的に解放する（EASY=接近＋攻撃のみ／NORMAL=＋ガード・投げ崩し／HARD=＋投げ抜け・ダッシュ接近・無敵対空＝全反応）。既定は HARD（従来挙動）。判断は相手の観測状態と距離のみで**乱数なし＝決定的**（入力リプレイと両立）。AI 自身のジャンプ・しゃがみ系は将来拡張
