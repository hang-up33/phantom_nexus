...
| 175 | BGM（バックグラウンドミュージック）：タイトル/バトル/リザルト用 OGG を再生・ループ・F6 でミュートトグル・画面遷移で自動切替（`GameRuntime/Audio/MusicManager`） | ✅ 完了 |
| 176 | アーケードモード（追加機能）：タイトル画面に ARCADE を追加・ランダムキャラ/ステージで 10 連戦のストーリー対戦・クリア＋敗退メッセージ・通常対戦/トレーニングと完全独立 | ✅ 完了 |
| 177 | セッション戦績（追加機能）：アプリ起動中のマッチ結果を P1W-L / P2W-L 形式で累積し決着バナーに表示（非撮影/非リプレイ時のみ・決定性不変） | ✅ 完了 |
| 178 | リマッチ（追加機能）：決着後に R キーで同じキャラ/ステージ/モードで即再戦・黄色の「PRESS R TO REMATCH」ヒントを結果バナーに表示（通常プレイのみ・後方互換） | ✅ 完了 |
| 179 | ランダムキャラ選択（追加機能）：キャラ選択画面で R キーを押すとカーソルをランダム位置に移動→ ENTER で確定（「R : random」ヒントをキー説明に追加） | ✅ 完了 |
